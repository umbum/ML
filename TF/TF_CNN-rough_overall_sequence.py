...
batch_size = 100  
epochs = 15
keep_prob = tf.placeholder(tf.float32) # == dropout rate. 0.5~0.7이 적당하고 test 시 항상 1을 줘야 하는 것에 주의!
...
for epoch in range(2):# range(epochs):
    avg_cost = 0
    
    for i in range(total_batch):
        batch_xs, batch_ys = mnist.train.next_batch(batch_size)
        feed_dict = {X: batch_xs, Y: batch_ys, keep_prob: 0.7}
        c, _, logit = sess.run([cost, optimizer, logits], feed_dict=feed_dict)
...
feed_dict={X:mnist.test.images[:1000], Y: mnist.test.labels[:1000], keep_prob:1}))

print("===== is Label == Prediction TEST=====")
r = random.randint(0, mnist.test.num_examples -1)
print("Label      : ", sess.run(tf.argmax(mnist.test.labels[r:r + 1], axis=1)))
print("Prediction : ", sess.run(tf.argmax(logits, axis=1), feed_dict={X: mnist.test.images[r:r + 1], keep_prob:1}))
