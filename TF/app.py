...
class CNN:
    __slots__ = ('sess', 'name', 'training', 'X', 'Y', 'logits', 'cost', 'optimizer', 'accuracy', 'merged_summary')
...
def _main():
    ...
    for m in range(num_models):
        models.append(CNN(sess, "model" + str(m)))    # _build_net이 호출되면서 graph에 알아서 노드 추가.
    
    writer = tf.summary.FileWriter('./tflogs/log-0423', graph=sess.graph)
